...
export class GroupedMessageByDatePipe implements PipeTransform {
  ...
  transform(messages: IMessage[]): { date: string; messages: IMessage[] }[] {
    ...
    return Object.keys(messagesByDate)
      ...
      .sort((a, b) => {
        const dateA = a.date === 'Сегодня' ? today : a.date === 'Вчера' ? yesterday : new Date(a.messages[0].createdAt);
        const dateB = b.date === 'Сегодня' ? today : b.date === 'Вчера' ? yesterday : new Date(b.messages[0].createdAt);
